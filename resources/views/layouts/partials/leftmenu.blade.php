        ...
        <div class="app-menu navbar-menu border-end">
            ...
            <div class="navbar-brand-box">
                ...
                <a href="/home" class="logo logo-dark">
                    <span class="logo-sm">
                        <img src="/images/logo/logo-bandes-mini.png" alt="" height="30">
                    </span>
                    <span class="logo-lg">
                        <img src="/images/logo/logo-bandes.png" alt="" height="32">
                    </span>
                </a>
                <!-- Light Logo-->
            ...
            <div id="scrollbar">
                <div class="container-fluid">
                    ...
                    <ul class="navbar-nav" id="navbar-nav">
                        <li class="menu-title"><span data-key="t-menu">Opciones</span></li>
                        <li class="nav-item">
                            <a class="nav-link menu-link  {{ Route::is('home') ? 'active' : '' }}" href="#sidebarDashboards" data-bs-toggle="collapse" role="button"  aria-expanded="{{ Route::is('home') ? 'true' : 'false' }}" aria-controls="sidebarDashboards">
                                <i class="mdi mdi-home"></i> <span data-key="t-dashboards">Inicio</span>
                            </a>
                            <div class="menu-dropdown collapse {{ Route::is('home') ? 'show' : 'false' }}" id="sidebarDashboards">
                                <ul class="nav nav-sm flex-column">
                                    <li class="nav-item">
                                        <a href="/home" class="nav-link {{ active_check(['home']) }}" data-key="t-analytics"> Dashboard </a>
                                    </li>
                         ...
                         <li class="nav-item">
                            <a class="nav-link menu-link " href="#sidebarSeguridad" data-bs-toggle="collapse" role="button"  aria-expanded="{{ Route::is('usuarios.*') ? 'true' : 'false' }} {{ Route::is('roles') ? 'true' : 'false' }} {{ Route::is('logins') ? 'true' : 'false' }}
                            {{ Route::is('empresa') ? 'true' : 'false' }}" aria-controls="sidebarSeguridad">
                                <i class="mdi mdi-shield"></i> <span data-key="t-dashboards">Seguridad</span>
                            </a>
                            <div class="menu-dropdown collapse {{ Route::is('usuarios.*') ? 'show' : 'false' }} {{ Route::is('roles') ? 'show' : 'false' }} {{ Route::is('logins') ? 'show' : 'false' }}  {{ Route::is('empresa.*') ? 'show' : 'false' }} {{ Route::is('logs.*') ? 'show' : 'false' }}" id="sidebarSeguridad">
                                <ul class="nav nav-sm flex-column">
                                   @can('acceso_usuarios')
                                   <li class="nav-item">
                                    <a href="/usuarios" class="nav-link {{ Route::is('usuarios.*') ? 'show' : 'false' }}" data-key="t-analytics"> Usuarios </a>
                                </li>
                                   @endcan
                                    @can('acceso_role')
                                    <li class="nav-item">
                                        <a href="/roles" class="nav-link {{ Route::is('roles.*') ? 'show' : 'false' }}" data-key="t-analytics"> Roles </a>
                                    </li>
                                    @endcan
                                    @can('acceso_empresa')
                                    <li class="nav-item">
                                        <a href="/empresa" class="nav-link {{ Route::is('empresa.*') ? 'show' : 'false' }}" data-key="t-analytics"> Ente </a>
                                    </li>
                                    @endcan
                                   @can('acceso_logins')
                                   <li class="nav-item">
                                    <a href="/logins" class="nav-link {{ Route::is('logjns.*') ? 'show' : 'false' }}" data-key="t-analytics"> Historial de logueo </a>
                                </li>
                                   @endcan
                                     @can('acceso_logins')
                                     <li class="nav-item">
                                        <a href="/logs" class="nav-link {{ Route::is('logs.*') ? 'show' : 'false' }}" data-key="t-analytics"> Logs del sistema </a>
                                    </li>
                                     @endcan
                                </ul>
                            </div>
                        ...
                        <li class="nav-item">
                            <a class="nav-link menu-link " href="#sidebarConfiguracion" data-bs-toggle="collapse" role="button"  aria-expanded="{{ Route::is('gerencia.*') ? 'true' : 'false' }} {{ Route::is('roles') ? 'true' : 'false' }} {{ Route::is('personal.*') ? 'true' : 'false' }}
                            {{ Route::is('producto.*') ? 'true' : 'false' }}" aria-controls="sidebarConfiguracion">
                                <i class="mdi mdi-cog"></i> <span data-key="t-dashboards">Configuración</span>
                            </a>
                            <div class="menu-dropdown collapse {{ Route::is('gerencia.*') ? 'show' : 'false' }} {{ Route::is('roles') ? 'show' : 'false' }} {{ Route::is('personal.*') ? 'show' : 'false' }}  {{ Route::is('producto.*') ? 'show' : 'false' }} {{ Route::is('logs.*') ? 'show' : 'false' }}" id="sidebarConfiguracion">
                                <ul class="nav nav-sm flex-column">
                                    @can('acceso_gerencia')
                                    <li class="nav-item">
                                        <a href="/gerencia" class="nav-link {{ Route::is('gerencia.*') ? 'show' : 'false' }}" data-key="t-analytics"> Gerencias </a>
                                    </li>
                                    @endcan
                                    @can('acceso_personal')
                                    <li class="nav-item">
                                        <a href="/personal" class="nav-link {{ Route::is('personal.*') ? 'show' : 'false' }}" data-key="t-analytics"> Personal </a>
                                    </li>
                                    @endcan

                                    <li class="nav-item">
                                        <a href="/producto" class="nav-link {{ Route::is('producto.*') ? 'show' : 'false' }}" data-key="t-analytics"> Productos </a>
                                    </li>
                        ...
                        <li class="menu-title"><i class="ri-more-fill"></i> <span data-key="t-pages">Registro de datos</span></li>



                       <li class="nav-item">
                            <a class="nav-link menu-link " href="#sidebarEntregaOperativo" data-bs-toggle="collapse" role="button"  aria-expanded="{{ Route::is('operativo.*') ? 'true' : 'false' }} {{ Route::currentRouteNamed('historial') ? 'true' : 'false' }}" aria-controls="sidebarEntregaOperativo">
                                <i class="mdi mdi-human-male"></i> <span data-key="t-dashboards">Operativo</span>
                            </a>
                            <div class="menu-dropdown collapse {{ Route::is('operativo.*') ? 'show' : 'false' }}" id="sidebarEntregaOperativo">
                                <ul class="nav nav-sm flex-column">
                                    @if (\Auth::user()->hasRole('Tecnologia') || \Auth::user()->hasRole('Seguridad') || \Auth::user()->hasRole('Operador'))
                                    <li class="nav-item">
                                        <a href="/operativo" class="nav-link {{ Route::is('operativo.*') ? 'show' : 'false' }}" data-key="t-analytics"> Entrega </a>
                                    </li>

                                    @endif
                                    @if (\Auth::user()->hasRole('Tecnologia') || \Auth::user()->hasRole('Seguridad'))
                                    <li class="nav-item active">
                                        <a href="/historial" class="nav-link " data-key="t-analytics"> Historial de entrega </a>
                                    </li>
                                    @endif
                                    @if (\Auth::user()->hasRole('Tecnologia') || \Auth::user()->hasRole('Seguridad'))
                                    <li class="nav-item active">
                                        <a href="/resultado" class="nav-link " data-key="t-analytics"> Gráficos </a>
                                    </li>
                                    @endif
